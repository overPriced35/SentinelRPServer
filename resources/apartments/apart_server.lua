...
AddEventHandler('house:updatespawns', function(info, hid)
    ...
    local table = info
    local house_id = hid
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE house_id = ?', {hid}, function(result)
        exports.ghmattimysql:execute("UPDATE houses SET `data` = @data WHERE `house_id` = @house_id", {
...
AddEventHandler('house:purchasehouse', function(cid,house_id,house_model,upfront,price,housename,garages)
    local src = source
    exports.ghmattimysql:execute('SELECT `housename` FROM houses WHERE `housename`= ?', {housename}, function(data)
        local penis = data[1].cid
        if penis == nil then
            exports.ghmattimysql:execute("UPDATE houses SET `cid` = @cid, `upfront` = @upfront, `housename` = @housename, `garages` = @garages, `house_model` = @house_model, `overall` = @overall, `payments` = @payments, `due` =@due, `days` = @days WHERE `house_id` = @house_id", {
                ['@cid'] = cid,
                ['@upfront'] = upfront,
                ['@housename'] = housename,
...
AddEventHandler('housing:requestSpawnTable', function(table, id, house_name)
    local src = source
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE `house_id` = @house_id', { ['@house_id'] = id }, function(result)
        if result[1] ~= nil then
            TriggerClientEvent('UpdateCurrentHouseSpawns', src, id, result[1].data)
        else
            exports.ghmattimysql:execute("INSERT INTO houses (data, house_id, housename) VALUES (@data, @house_id, @housename)", {
            ['@house_id'] = id,
            ['@data'] = json.encode(table),
...
AddEventHandler('housing:getGarage', function(house_id, house_model)
    ...
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE `house_id`= ? AND `house_model`= ?  AND `cid` = ?', {house_id, house_model, player.id}, function(penis)
        if penis[1] ~= nil then
...
AddEventHandler('house:enterhouse', function(cid,isRealEstate,house_id,house_model,furniture)
    local src = source
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(data)
    ...
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(data3)
            ...
            exports.ghmattimysql:execute('SELECT * FROM houses WHERE `cid`= ?', {cid}, function(data)
                ...
                exports.ghmattimysql:execute('SELECT * FROM __housekeys WHERE `cid`= ?', {cid}, function(returnData)
                    for k, v in pairs(returnData) do
                        if v.house_id == house_id then
                            exports.ghmattimysql:execute('SELECT * FROM houses WHERE `house_id`= ?', {v.house_id}, function(penis)
...
AddEventHandler('house:givekey', function(house_id,house_model,house_name,target)
    ...
    exports.ghmattimysql:execute('SELECT `house_id` FROM __housekeys WHERE `cid`= ?', {cid}, function(data)
        ...
        if penis == "[]" then
            exports.ghmattimysql:execute('SELECT `garages` FROM houses WHERE `house_id`= ?', {house_id}, function(test)
...
AddEventHandler("housing:unlockRE", function(house_id, house_model)
    local src = source
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE house_id = ?', {house_id}, function(result)
        local address = result[1].housename
        if result[1].force_locked == "unlocked" then
            exports.ghmattimysql:execute("UPDATE houses SET `force_locked` = @force_locked WHERE `house_id` = @house_id ", {
                ['@force_locked'] = "locked",
                ['@house_id'] = house_id
            })
...
AddEventHandler("housing:unlock", function(house_id, house_model)
    ...
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE house_id = ?', {house_id}, function(result)
        ...
        if result[1].status == "unlocked" then
            exports.ghmattimysql:execute("UPDATE houses SET `status` = @status WHERE `house_id` = @house_id ", {
                ['@status'] = "locked",
                ['@house_id'] = house_id
            })
...
AddEventHandler('CheckFurniture', function(house_id, house_model)
    local src = source
    exports.ghmattimysql:execute('SELECT `furniture` FROM houses WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(data)
...
AddEventHandler('house:evictHouse', function(hid, model)
    local src = source
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE house_id = ? AND house_model = ?', {hid, model}, function(result)
        if result[1] ~= nil then
            TriggerClientEvent("DoLongHudText", src, result[1].house_name.. " has been deleted", 2)
            exports.ghmattimysql:execute("DELETE FROM houses WHERE house_id = @house_id AND house_model = @house_model", {['house_id'] = hid, ['house_model'] = model})
        end
    end)
end)
...
AddEventHandler("house:PropertyOutstanding", function(house_id, house_model)
    local src = source
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE house_id = ? AND house_model = ?', {house_id, house_model}, function(result)
        if result[1] ~= nil then
...
AddEventHandler("housing:garagesSET", function(garages, house_id)
    local src = source
    exports.ghmattimysql:execute("UPDATE houses SET `garages` = @garages WHERE `house_id` = @house_id", {
        ['@garages'] = json.encode(garages),
        ['@house_id'] = house_id
    })
                ...
                if result[1].can_pay ~= "false" then
                    if result[1].due ~= tonumber(0) then
                        if player.cash >= amountdue then
                            ...
                            exports.ghmattimysql:execute("UPDATE houses SET `payments` = @payments, `due` = @due, `days` = @days, `can_pay` = @can_pay WHERE `house_id` = @house_id AND `house_model` = @house_model", {
...
function updateFinance()
    exports.ghmattimysql:execute('SELECT days, house_id FROM houses WHERE days > @days', {
        ["@days"] = 0
    }, function(result)
        for i=1, #result do
            ...
            if days ~= nil then
                exports.ghmattimysql:execute('UPDATE houses SET days = @days WHERE house_id = @house_id', {
                    ['@house_id'] = house_id,
                    ['@days'] = newTimer
                })
            end
            if days < 100 then
                exports.ghmattimysql:execute("UPDATE houses SET `can_pay` = @can_pay WHERE `house_id` = @house_id ", {
                    ['@can_pay'] = "true",
                    ['@house_id'] = house_id
                })
            end
...
AddEventHandler('housing:nearHouses', function(id)
    exports.ghmattimysql:execute('SELECT * FROM houses_ houses house_id = @house_id', {['house_id'] = id}, function(result)
